...
insertDataName=function(data.name,parent,loc="last"){
    # data.name "character"
    # parent "gtkTreeView"
    ...
    # insert data.name
        print(data.name[i])
    parent$getModel()$setValue(iter=iter,column=0,value=data.name[i])
...
# data.frame(test)
# 
# insertDataName(data.name=c("test1","test2"),parent=appspace[active.view],loc="insert")
# insertDataName(data.name=test,parent=appspace[active.view],loc="insert")
# 
# 
# len=length(test)
# 
# for (i in 1:len) insertDataName(data.name=test[i],parent=appspace[active.view],loc="insert")
